    ...
    xml.service(:version => "1", :name => "system/config-user", :type => "service") {
      xml.instance(:enabled => "true", :name => "default") {
        ...
        xml.property_group(:name => "user_account", :type => "application") {
          ...
          xml.propval(:value => $q_struct["admin_uid"].value, :name => "uid")
          xml.propval(:value => $q_struct["admin_gid"].value, :name => "gid")
          xml.propval(:type => "astring", :value => $q_struct["admin_type"].value, :name => "type")
          xml.propval(:type => "astring", :value => $q_struct["admin_roles"].value, :name => "roles")
          xml.propval(:type => "astring", :value => $q_struct["admin_profiles"].value, :name => "profiles")
          xml.propval(:type => "astring", :value => $q_struct["admin_sudoers"].value, :name => "sudoers")
          xml.propval(:type => "astring", :value => $q_struct["admin_expire"].value, :name => "expire")
        }
      }
    }
    ...
    xml.service(:version => "1", :name => "system/config-user") {
      xml.instance(:enabled => "true", :name => "default") {
        ...
        xml.property_group(:name => "user_account") {
          ...
          xml.propval(:type => "astring", :value => $q_struct["admin_sudoers"].value, :name => "sudoers")
          xml.propval(:type => "astring", :value => $q_struct["admin_expire"].value, :name => "expire")
          #xml.propval(:type => "astring", :value => $q_struct["admin_home_zfs_dataset"].value, :name => "home_zfs_dataset")
          #xml.propval(:type => "astring", :value => $q_struct["admin_home_mountpoint"].value, :name => "home_mountpoint")
        }
      }
    }
...
def create_ai_manifest(output_file)
  ...
  xml.auto_install {
    xml.ai_instance(:auto_reboot => "true", :name => "orig_default") {
      xml.target {
        xml.logical {
          xml.zpool(:is_root => "true", :name => "rpool") {
      ...
      xml.software(:type => "IPS") {
        ...
        xml.source {
          xml.publisher(:name => "solaris") {
            xml.origin(:name => $q_struct["publisher_url"].value)
          }
        }
