    ...
    xml.service(:version => "1", :name => "system/config-user") {
      xml.instance(:enabled => "true", :name => "default") {
        ...
        xml.property_group(:name => "user_account") {
          ...
          #xml.propval(:type => "astring", :value => q_struct["account_home_zfs_dataset"].value, :name => "home_zfs_dataset")
          #xml.propval(:type => "astring", :value => q_struct["account_home_mountpoint"].value, :name => "home_mountpoint")
        }
      }
    }
...
def create_ai_manifest(q_struct,output_file)
  xml_output=[]
  xml=Builder::XmlMarkup.new(:target => xml_output, :indent => 2)
